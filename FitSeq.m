function [ file_name, x_estimate_result, r_estimate_result, x_mean_est] = ...
    FitSeq(BC_num_mat_original, t_seq_vec, cell_depth, varargin)
%--------------------------------------------------------------------------
% FitSeq
...
%
% -- cell_depth: a matrix of the cell number of the population after bottleneck 
%                (cells transferred at the bottleneck), and before bottleneck
%                at each sequencing time point, use [] as input if it is unknown
%                size = 2 * (length(t_seq_vec)-1)
%
...
%                 optimization method to estimate fitness, default value is 10
%
...
vararg_update = {'format','csv', 'kappa',2.5, 'opt_cycle_max',10};
...
while (likelihood_all_new  >= likelihood_all_old) && (opt_cycle_num_count < opt_cycle_num_max)
    ...
    if  (likelihood_all_new < likelihood_all_old) && opt_cycle_num_count == 1
        ...
        x_mean_est_result = x_mean_est_result_linear;
...
switch lower(ouptput_format)
    case {'mat'}
        ...
        save(file_name,'file_name','x_estimate_result','r_estimate_result',...
            'x_mean_est_result','likelihood_all_result','opt_cycle_num_count')
    case {'csv'}
        ...
        csvwrite(file_name_3, x_mean_est_result)
